...
```bash
brew install torsday/tap/little-snitch-mcp
```

That installs the same notarized binary distributed on the GitHub Release — Homebrew just handles the download, the architecture pick, and future `brew upgrade`s. The formula lives in [`torsday/homebrew-tap`](https://github.com/torsday/homebrew-tap) and is auto-bumped by this repo's release workflow.

> **Pre-v1.0:** the formula isn't published until `v1.0.0` ships. Until then, use the binary download below.

### Binary download

For users who'd rather skip Homebrew, or who need a pre-release build:

1. Download the notarized archive from the [latest release](https://github.com/torsday/little-snitch-mcp/releases/latest) for your architecture:
   - `little-snitch-mcp-v<version>-aarch64-apple-darwin.tar.gz` — Apple Silicon (M1/M2/M3/M4)
   - `little-snitch-mcp-v<version>-x86_64-apple-darwin.tar.gz` — Intel
...
```bash
tar -xzf little-snitch-mcp-*.tar.gz
sudo mv little-snitch-mcp /usr/local/bin/
```

The binary is signed with a Developer ID Application certificate, notarized by Apple, and ships with a Sigstore-signed [build-provenance attestation](https://docs.github.com/en/actions/security-guides/using-artifact-attestations). Gatekeeper validates the signature offline; you can verify provenance with `gh attestation verify`. See [Security & trust → Verify it yourself](#verify-it-yourself) for the full recipe.

### Building from source

For contributors and Rust developers who want a local build, see [`CONTRIBUTING.md`](./CONTRIBUTING.md). Note that a locally-built binary is **not** notarized — Gatekeeper will require an explicit override on first run.
...
- **Two-step confirmation protocol for every live mutation** — no single LLM prompt can change the live model. The `prepare_*` tool reads the current model, computes a diff hash, and issues an HMAC-SHA256 token (60-second TTL — see `safety::token::DEFAULT_TTL_SECS`); the corresponding apply tool re-reads the model, recomputes the hash, and refuses if anything drifted. Implementation in [`src/safety/token.rs`](./src/safety/token.rs); the seven verifier checks are pinned by ADR-0004 §9 and the eight-row test matrix.
...
- [`docs/adr/0004-safety-permissions-and-confirmation.md`](./docs/adr/0004-safety-permissions-and-confirmation.md) — full safety model: tier taxonomy, token protocol, seven verifier checks
...
- [MIT License](./LICENSE-MIT) ([summary](https://choosealicense.com/licenses/mit/))
- [Apache License, Version 2.0](./LICENSE-APACHE) ([summary](https://choosealicense.com/licenses/apache-2.0/))

This is the standard Rust-ecosystem dual license. Pick whichever fits your project; you don't need to comply with both.
