...
| Confirmation-token forgery | Predictable / replayable / TTL-bypassable tokens let a single LLM call mutate the live model. | HMAC-SHA256 over (tool, args-canonicalized, model-state-hash, expiry); `subtle::ConstantTimeEq` for verification; 60-second TTL (`safety::token::DEFAULT_TTL_SECS`); bound to a model-state hash that's recomputed at apply time. | ADR-0004 § 9 (seven verifier checks; eight-row acceptance matrix) and `src/safety/token.rs` tests. |
