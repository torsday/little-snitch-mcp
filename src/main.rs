...
/// Serialize a value as pretty JSON inside a single-element
/// `ReadResourceResult` text envelope — the wire shape every read-resource
/// branch in `ServerHandler::read_resource` produces.
///
/// On serialization failure the error is surfaced via `McpError::internal_error`
/// (the previous per-branch boilerplate did the same; centralizing it here
/// removes four near-identical copies and gives the response shape one place
/// to evolve).
fn text_resource_result<T: serde::Serialize>(
    value: &T,
...
/// Convert a tool's `Result<T: Serialize, String>` into the MCP wire shape
/// every `#[tool]` method on this server returns:
///
/// - `Ok(value)` → JSON-serialized `Content::text(...)` inside a `success`.
/// - `Err(msg)`  → `CallToolResult::error` carrying the message.
/// - A serialization failure on `Ok(value)` is *also* surfaced as an error
///   (not silently swallowed into a "successful" body, as the prior
///   per-tool boilerplate did). Serialization failures here would indicate
///   a programmer bug in the result type, not an operational error — but
///   misrepresenting them as success is worse than reporting them.
///
/// Centralized here so the response envelope (pretty-print vs compact, error
/// shape, tracing hooks) has one place to evolve. Every tool body becomes
/// one line: `json_tool_result(tools::foo::run(args))`.
...
impl LittleSnitchServer {
    ...
    async fn validate_lsrules(
        ...
        json_tool_result(tools::validate_lsrules::run(args))
    }
    ...
    async fn create_lsrules_file(
        ...
        json_tool_result(tools::create_lsrules_file::run(args))
    }
    ...
    async fn remove_rule_from_lsrules_file(
        ...
        json_tool_result(tools::remove_rule_from_lsrules_file::run(args))
    }
    ...
    async fn update_rule_in_lsrules_file(
        ...
        json_tool_result(tools::update_rule_in_lsrules_file::run(args))
    }
    ...
    async fn add_rule_to_lsrules_file(
        ...
        json_tool_result(tools::add_rule_to_lsrules_file::run(args))
    }
    ...
    async fn export_model_backup(
        ...
        json_tool_result(tools::export_model_backup::run(args))
    }
    ...
    async fn list_backups(
        ...
        json_tool_result(tools::list_backups::run(args))
    }
    ...
    async fn read_preference(
        ...
        json_tool_result(tools::read_preference::run(args))
    }
    ...
    async fn list_preferences(
        ...
        json_tool_result(tools::list_preferences::run(args))
    }
    ...
    async fn doctor(
        ...
        json_tool_result(tools::doctor::run(args))
    }
    ...
    async fn tail_log(
        ...
        json_tool_result(tools::tail_log::run(args))
    }
    ...
    async fn warm_sudo(
        ...
        json_tool_result(tools::warm_sudo::run(args).await)
    }
    ...
    async fn show_restrictions(
        ...
        json_tool_result(tools::show_restrictions::run(args))
    }
    ...
    async fn prepare_write_preference(
        ...
        json_tool_result(tools::write_preference::prepare_write(&self.session, args))
    }
    ...
    async fn write_preference(
        ...
        json_tool_result(tools::write_preference::write(&self.session, args))
    }
    ...
    async fn prepare_remove_preference(
        ...
        json_tool_result(tools::write_preference::prepare_remove(&self.session, args))
    }
    ...
    async fn remove_preference(
        ...
        json_tool_result(tools::write_preference::remove(&self.session, args))
    }
    ...
    async fn tail_traffic(
        ...
        json_tool_result(tools::tail_traffic::run(args))
    }
    ...
    async fn set_lsrules_metadata(
        ...
        json_tool_result(tools::lsrules_metadata::set_metadata(args))
    }
    ...
    async fn diff_lsrules_files(
        ...
        json_tool_result(tools::lsrules_metadata::diff_files(args))
    }
    ...
    async fn capture_process_traffic(
        ...
        json_tool_result(tools::capture_process_traffic::run(args).await)
    }
    ...
    async fn prepare_activate_profile(
        ...
        json_tool_result(tools::manage_profiles::prepare_activate(
            &self.session,
            args,
        ))
    }
    ...
    async fn activate_profile(
        ...
        json_tool_result(tools::manage_profiles::activate(&self.session, args))
    }
    ...
    async fn prepare_deactivate_all_profiles(
        ...
        json_tool_result(tools::manage_profiles::prepare_deactivate(
            &self.session,
            args,
        ))
    }
    ...
    async fn deactivate_all_profiles(
        ...
        json_tool_result(tools::manage_profiles::deactivate_all(&self.session, args))
    }
    ...
    async fn prepare_update_factory_rule_groups(
        ...
        json_tool_result(tools::update_factory_rule_groups::prepare_update(
            &self.session,
            args,
        ))
    }
    ...
    async fn update_factory_rule_groups(
        ...
        json_tool_result(tools::update_factory_rule_groups::update(
            &self.session,
            args,
        ))
    }
    ...
    async fn prepare_enable_rule_group(
        ...
        json_tool_result(tools::manage_rule_groups::prepare_enable(
            &self.session,
            args,
        ))
    }
    ...
    async fn enable_rule_group(
        ...
        json_tool_result(tools::manage_rule_groups::enable(&self.session, args))
    }
    ...
    async fn prepare_disable_rule_group(
        ...
        json_tool_result(tools::manage_rule_groups::prepare_disable(
            &self.session,
            args,
        ))
    }
    ...
    async fn disable_rule_group(
        ...
        json_tool_result(tools::manage_rule_groups::disable(&self.session, args))
    }
    ...
    async fn get_rules_for_process(
        ...
        json_tool_result(tools::get_rules_for_process::run(args))
    }
    ...
    async fn find_rules_for_remote(
        ...
        json_tool_result(tools::find_rules_for_remote::run(args))
    }
}

#[tool_handler]
#[prompt_handler(router = self.prompt_router)]
impl ServerHandler for LittleSnitchServer {
    ...
    async fn read_resource(
        ...
        if request.uri == resources::rule_groups::URI {
            ...
            return text_resource_result(&summaries, resources::rule_groups::URI);
        }
        ...
        if let Some(id) = resources::rule_groups::match_item_uri(&request.uri) {
            ...
            match resources::rule_groups::get_group(id, &model) {
                Some(detail) => return text_resource_result(&detail, request.uri),
                None => {
        ...
        if request.uri == resources::lsrules_files::URI {
            let entries = resources::lsrules_files::list(&managed.rules)
                .map_err(|e| McpError::internal_error(e, None))?;
            return text_resource_result(&entries, resources::lsrules_files::URI);
        }
        ...
        if let Some(name) = resources::lsrules_files::match_file_uri(&request.uri) {
            match resources::lsrules_files::read_file(&managed.rules, name) {
                Ok(contents) => return text_resource_result(&contents, request.uri),
...
async fn main() -> Result<()> {
    ...
    let service = LittleSnitchServer::new(session).serve(stdio()).await?;
